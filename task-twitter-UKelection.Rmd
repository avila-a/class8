...
Starting to include party names to compare them
```{r}
...
corp_la <- corpus(labour_tweets)
corp_co <- corpus(conservative_tweets)

# add doc variable
docvars(corp_jc, "subject") <- "Corbyn"
docvars(corp_bj, "subject") <- "Boris"
docvars(corp_la, "subject") <- "Labour"
docvars(corp_co, "subject") <- "Conservative"

# Add them toguether
corpus_data <- corp_bj + corp_jc + corp_co + corp_la
...
dfm_mat <- data.frame(matrix(dfm@x, nrow = 4, dimnames = dfm@Dimnames))
dfm_mat$subject <- dfm@Dimnames[[1]]
dfm_mat
```
...
```{r}
library(tidyverse)
dta_long <- pivot_longer(dfm_mat[, c(1,2,5)], cols = c("negative", "positive")
             , names_to = "Sentiment")
dta_long

ggplot(data = dta_long, aes(x = subject, y = value, fill = Sentiment)) +
...
dta_long$party <- ifelse(test = dta_long$subject %in% 
                           c("Boris", "Conservative"),
                         yes = "Conservative", 
                         no = "Labour")

dta_long$tw_account <- ifelse(test = dta_long$subject %in% 
                           c("Boris", "Corbyn"),
                         yes = "candidate", 
                         no = "party")

ggplot(data = dta_long, aes(x = tw_account, y = value, 
                            fill = Sentiment)) +
  geom_bar(stat = 'identity', position = 'fill') +
  facet_grid(. ~ party) +
  labs(y = "Proportion of tweets",
       x = "Twitter account") 
```
